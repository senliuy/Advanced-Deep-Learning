...
处理变长数据的一个非常经典的模型便是RNN，因为它可以将输入数据按照网络块切分成时间片。具体的讲，LSTM的输入是尺寸为$$4\times b$$超参数$$\mathcal{S}_b$$，其中4指的是超参数的四个元素$$(I_1, I_2, O_1, O_2)$$。输入LSTM之前，$$(I_1,I_2)$$会通过一个共享的嵌入层进行编码，$$(O_1,O_2)$$则会通过另外一个共享的嵌入层进行编码。最后的隐层节点经过一个激活函数为sigmoid的全连接得到最后的预测精度。
