...
假设样本有$$N$$个关键点，DenseBox的关键点检测的输出是$$N$$个热图，热图中的每个像素点表示改点为对应位置关键点的置信度。

Landmark的Ground Truth生成方式也很简单，对于标签集中的第i个关键点$$(x,y)$$，在第i个feature map在$$(x,y)$$处的值是1，其它位置为0，有时也可以以$$(x,y)$$为圆心将值为1的区域扩展成一个圆，半径为$$r_l$$。

Landmark使用了1.4节中介绍的灰色区域和Hard Negative Mining方法进行采样，损失函数则是使用了采样样本之间的l2损失函数。整个关键点检测如图4中左半部分所示。
