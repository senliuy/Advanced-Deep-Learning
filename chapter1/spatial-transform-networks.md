...
2. 理论上讲ST是可以以任意数量插入到网络中的任意位置，多个ST可以起到裁剪的作用，是一种高级的Attention机制。但多个ST无疑增加了网络的深度，其带来的收益价值值得讨论；
...
在鸟类分类的任务上，作者并行使用了4个ST，得到了图5的实验解雇：



### 3.2 STN用于物体检测

在这个场景中，STN构造Triplet Loss损失函数进行检测的，具体的说，我们知道多个ST有裁剪的作用，那么这个任务的优化目标便是使随机位置减去Ground Truth尽可能大于裁剪的坐标减去Ground Truth，如图4。
