...
在这个场景中，输入是两张有仿射变换的MNIST的图片，然后直接输出这两个图片的数字的和（是一个19类的分类任务，不是两个10分类任务），如图3。

###### 图3：并行STM

![](/assets/STN_3.png)

具体的将，给定两张图像，初始化两个STM，将两个STM分别作用于两张图片，得到四个Feature Map，将这个四个通道的图片作为FCN的输入，预测0-18间的一个整数值。
