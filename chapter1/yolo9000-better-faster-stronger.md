...
其中$$\hat{g_j^{cx}}$$和$$\hat{g}_j^{cy}$$是预测值，在模型训练初期，由于使用的是迁移学习和随机初始化得到的网络，此时网络必然收敛的不是特别好，这就造成了$$\hat{g}^{cx}_j$$和$$\hat{g}^{cy}_j$$的随机性。再加上并没有对$$\hat{g}$$的值加以限制，使得预测的检测卡可能出现在网络中的任何位置而且这些事和锚点如何初始化无关的。正确的做法应该是预测的检测框应该也是由该锚点负责的，也就预测的检测框的中心点应该落到该锚点的内部。
