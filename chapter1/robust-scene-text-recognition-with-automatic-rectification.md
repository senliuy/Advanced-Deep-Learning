...
场景文字检测的难点有很多，仿射变换（图2.\(a\)）是其中一种，Jaderberg\[2\]等人提出的STN通过预测仿射变换矩阵的方式对输入图像进行矫正。但是真实场景的不规则文本要复杂的多，可能包括扭曲，弧形排列等情况（图2.\(b\)）,这种方式的变换是传统的STN解决不了的，因此作者提出了基于TPS的STN，RARE的STN也是可微分的，所以RARE也是一个可end-to-end训练的算法。

###### 图2：自然场景中的变换：(a)仿射变换，(b)弧形排列
